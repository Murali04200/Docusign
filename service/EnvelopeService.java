...
import com.example.Docusign.mail.EnvelopeMailService;
...
public class EnvelopeService {
    ...
    public Envelope sendEnvelope(String envelopeId) {
        ...
        // Mark first routing order recipients as delivered and send emails
        ...
        for (EnvelopeRecipient recipient : recipients) {
            if (recipient.getRoutingOrder() == firstOrder) {
                ...
                try {
                    envelopeMailService.sendSignatureRequest(
                        recipient.getEmail(),
                        recipient.getName(),
                        envelope.getSenderUserId(),
                        envelope.getName(),
                        envelope.getSubject(),
                        envelope.getMessage(),
                        envelopeId
                    );
                    log.info("Sent signature request email to: {}", recipient.getEmail());
                } catch (Exception e) {
                    log.error("Failed to send signature request email to: {}", recipient.getEmail(), e);
                }
            }
        }
    ...
    private void checkAndCompleteEnvelope(String envelopeId) {
        ...
        if (allCompleted) {
            ...
            if (envOpt.isPresent()) {
                ...
                try {
                    envelopeMailService.sendCompletionNotification(
                        envelope.getSenderUserId(),
                        envelope.getName(),
                        envelopeId
                    );
                    log.info("Sent completion notification email to sender: {}", envelope.getSenderUserId());
                } catch (Exception e) {
                    log.error("Failed to send completion notification email", e);
                }
            }
        }
    }
    ...
    public Envelope voidEnvelope(String envelopeId, String reason) {
        ...
        for (EnvelopeRecipient recipient : recipients) {
            ...
            // Send voided email to recipient
            try {
                envelopeMailService.sendVoidedNotification(
                    recipient.getEmail(),
                    recipient.getName(),
                    envelope.getName(),
                    reason
                );
                log.info("Sent voided notification email to: {}", recipient.getEmail());
            } catch (Exception e) {
                log.error("Failed to send voided notification email to: {}", recipient.getEmail(), e);
            }
        }
